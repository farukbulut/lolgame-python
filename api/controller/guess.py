...
def make_ability_guess(request):
    ...
    if request.method == 'POST':
        try:
            ...
            if not ability_id or not game_id:
                return JsonResponse({'error': 'Missing parameters'}, status=400)
            ...
            # Get the guessed ability
            try:
                guessed_ability = Ability.objects.get(id=ability_id)
            ...
            # Check if correct guess
            is_correct = (game.target_ability_id == int(ability_id))
            ...
            response_data = {
                ...
                'attempts_used': game.attempts_used,
                'max_attempts': game.game_mode.max_attempts,
                'feedback': prepare_ability_guess_feedback(game.target_ability, guessed_ability, language),
                'score': score if is_correct else 0,
                'user': anon_username if 'anon_user' in locals() else (request.user.username if hasattr(request, 'user') and request.user.is_authenticated else 'Anonymous')
            }

            # If game completed, add target ability details
            if game.is_completed:
                response_data['target_ability'] = get_ability_details(game.target_ability, language)
